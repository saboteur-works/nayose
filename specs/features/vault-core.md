...
- OQ-4: The provisional storage format (a non-goal to finalize here) still has to support the assertion log, the entity model, and the share/registration-state relationships well enough that Feature 2's export work does not force a full data migration. What level of format stability is expected from this feature before Feature 2 begins, given the format itself is explicitly out of this feature's scope? — Impact: FR-1 through FR-15 collectively, and the Feature 1 → Feature 2 dependency. — DEFERRED. The actionable guidance for this feature is unchanged in the meantime: keep the boundary between storage and everything above it clean.
  Becomes live at: Feature 2's gate, where FR-20 through FR-29 will define the target the provisional storage must reach.
